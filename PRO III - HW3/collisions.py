...
def createColSphere(obj, name, radius, show = False):
...
def createTube(obj, name, length, radius, show = False):
    #Create a collision sphere - return the collision node
    cNode = CollisionNode(name)
    cNode.addSolid(CollisionTube(0,-length/2,-15, 0, length/2, -15,radius))
    objC = obj.attachNewNode(cNode)
    if (show == True): objC.show()
    return objC

def createColRay(origin, dir, name):
